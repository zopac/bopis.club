...
public
class GuiBetterChat extends GuiNewChat {
    ...
    public
    GuiBetterChat ( Minecraft mcIn ) {
        super ( mcIn );
        this.mc = mcIn;
    }

    public static
    void updatePercentage ( long diff ) {
        if ( percentComplete < 1 ) percentComplete += 0.004f * diff;
        percentComplete = clamp ( percentComplete , 0 , 1 );
    }

    public static
    int calculateChatboxWidth ( float scale ) {
        int i = 320;
        int j = 40;
    ...
    public static
    int calculateChatboxHeight ( float scale ) {
        int i = 180;
        int j = 20;
        ...
        if ( this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN ) {
            ...
            if ( j > 0 ) {
                ...
                if ( BetterChat.getSettings ( ).smooth && ! this.isScrolled )
                    GlStateManager.translate ( 2.0F + BetterChat.getSettings ( ).xOffset , 8.0F + BetterChat.getSettings ( ).yOffset + ( 9 - 9 * percent ) * f1 , 0.0F );
                else
    ...
    private
    void setChatLine ( ITextComponent chatComponent , int chatLineId , int updateCounter , boolean displayOnly ) {
        if ( chatLineId != 0 ) {
     ...
     */
    public
    void addToSentMessages ( String message ) {
