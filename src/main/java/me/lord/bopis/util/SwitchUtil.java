...
public class SwitchUtil
    ...
    public static Map<Integer, ItemStack> getInventoryAndHotbarSlots() {
        if ( mc.currentScreen instanceof GuiCrafting) {
            return SwitchUtil.fuckYou3arthqu4kev2(10, 45);
        }
        return SwitchUtil.getInventorySlots(9, 44);
    }
    ...
    public static boolean[] switchItem(boolean back, int lastHotbarSlot, boolean switchedItem, Switch mode, Class clazz) {
        ...
        switch (mode) {
            ...
            case NONE: {
                switchedItemSwitched[1] = back ? true : mc.player.inventory.currentItem == InventoryUtil.findHotbarBlock(clazz);
            }
        }
    ...
    public static boolean[] switchItemToItem(boolean back, int lastHotbarSlot, boolean switchedItem, Switch mode, Item item) {
        ...
        switch (mode) {
            ...
            case NONE: {
                switchedItemSwitched[1] = back ? true : mc.player.inventory.currentItem == InventoryUtil.getItemHotbar(item);
            }
        }
        return switchedItemSwitched;
    }
    ...
    public static class Task {
        ...
        public void run() {
            ...
            if (this.slot != -1) {
                mc.playerController.windowClick( mc.player.inventoryContainer.windowId, this.slot, 0, this.quickClick ? ClickType.QUICK_MOVE : ClickType.PICKUP, (EntityPlayer) mc.player);
            }
        }
