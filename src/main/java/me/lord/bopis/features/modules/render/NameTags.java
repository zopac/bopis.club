...
public class NameTags extends Module {
    ...
    public void onRender3D( Render3DEvent event) {
        for (EntityPlayer player : Util.mc.world.playerEntities) {
            if (player != null && !player.equals( Util.mc.player) && player.isEntityAlive() && (!player.isInvisible() || invisibles.getValue())) {
                double x = interpolate(player.lastTickPosX, player.posX, event.getPartialTicks()) - Util.mc.getRenderManager().renderPosX;
                double y = interpolate(player.lastTickPosY, player.posY, event.getPartialTicks()) - Util.mc.getRenderManager().renderPosY;
                double z = interpolate(player.lastTickPosZ, player.posZ, event.getPartialTicks()) - Util.mc.getRenderManager().renderPosZ;
    ...
    private void renderNameTag(EntityPlayer player, double x, double y, double z, float delta) {
        ...
        double distance = camera.getDistance(x + Util.mc.getRenderManager().viewerPosX, y + Util.mc.getRenderManager().viewerPosY, z + Util.mc.getRenderManager().viewerPosZ);
        ...
        GlStateManager.rotate( Util.mc.getRenderManager().playerViewX, Util.mc.gameSettings.thirdPersonView == 2 ? -1.0F : 1.0F, 0.0F, 0.0F);
    ...
    private void renderItemStack(ItemStack stack, int x, int y) {
        ...
        Util.mc.getRenderItem().renderItemOverlays( Util.mc.fontRenderer, stack, x, y);
    ...
    private String getDisplayTag(EntityPlayer player) {
        ...
        if (ping.getValue()) {
            try {
                final int responseTime = Objects.requireNonNull( Util.mc.getConnection()).getPlayerInfo(player.getUniqueID()).getResponseTime();
