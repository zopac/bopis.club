...
import me.bopis.king.event.events.ConnectionEvent;
...
public class LogoutSpots
    ...
    public void onConnection(ConnectionEvent event) {
        if (event.getStage() == 0) {
            ...
            if (entity != null && this.message.getValue().booleanValue()) {
                Command.sendMessage(entity.getName() + " just logged in" + (this.coords.getValue() != false ? " at (" + (int) entity.posX + ", " + (int) entity.posY + ", " + (int) entity.posZ + ")!" : "!"));
            }
            ...
            if (this.message.getValue().booleanValue()) {
                Command.sendMessage(event.getName() + " just logged out" + (this.coords.getValue() != false ? " at (" + (int) entity.posX + ", " + (int) entity.posY + ", " + (int) entity.posZ + ")!" : "!"));
            }
    ...
    private void renderNameTag(String name, double x, double yi, double z, float delta, double xPos, double yPos, double zPos) {
        ...
        this.renderer.drawStringWithShadow(displayTag, -width, -(this.renderer.getFontHeight() - 1), this.colorSync.getValue() != false ? ClickGui.getInstance().getCurrentColorHex() : ColorUtil.toRGBA(new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue())));
