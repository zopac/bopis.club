...
import me.bopis.king.features.setting.EnumConverter;
import me.bopis.king.features.setting.Setting;
import me.bopis.king.mixin.mixins.accessors.IContainer;
import me.bopis.king.mixin.mixins.accessors.ISPacketSetSlot;
...
public class Offhand
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof SPacketSetSlot && (packet = event.getPacket()).getSlot() == -1 && packet.getWindowId() != -1) {
    ...
    private boolean noNearbyPlayers() {
        return this.mode == Mode.CRYSTALS && Offhand.mc.world.playerEntities.stream().noneMatch(e -> e != Offhand.mc.player && ! Bopis.friendManager.isFriend(e) && Offhand.mc.player.getDistance(e) <= this.cTargetDistance.getValue().floatValue());
    }
