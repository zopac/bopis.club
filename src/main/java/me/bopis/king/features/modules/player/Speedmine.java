...
import me.bopis.king.event.events.BlockEvent;
...
public class Speedmine
    ...
    private final Setting<Float> range = this.register(new Setting<Float>("Range", 10.0f, 0.0f, 50.0f));
    ...
    public Setting<Float> damage = this.register(new Setting<Object>("Damage", 0.7f, 0.0f, 1.0f, v -> this.mode.getValue() == Mode.DAMAGE && this.tweaks.getValue() != false));
    ...
    public Setting<Boolean> noGapTrace = this.register(new Setting<Object>("NoGapTrace", false, v -> this.noTrace.getValue()));
    public Setting<Boolean> pickaxe = this.register(new Setting<Object>("Pickaxe", true, v -> this.noTrace.getValue()));
    ...
    private final Setting<Integer> boxAlpha = this.register(new Setting<Object>("BoxAlpha", 85, 0, 255, v -> this.box.getValue() != false && this.render.getValue() != false));
    public Setting<Boolean> outline = this.register(new Setting<Object>("Outline", true, v -> this.render.getValue()));
    private final Setting<Float> lineWidth = this.register(new Setting<Object>("LineWidth", 1.0f, 0.1f, 5.0f, v -> this.outline.getValue() != false && this.render.getValue() != false));
    ...
    public void onTick() {
        if (this.currentPos != null) {
            ...
            if (Speedmine.mc.player != null && this.silentSwitch.getValue().booleanValue() && this.timer.passedMs((int) (2000.0f * Bopis.serverManager.getTpsFactor())) && this.getPickSlot() != -1) {
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.render.getValue().booleanValue() && this.currentPos != null) {
            Color color = new Color(this.timer.passedMs((int) (2000.0f * Bopis.serverManager.getTpsFactor())) ? 0 : 255, this.timer.passedMs((int) (2000.0f * Bopis.serverManager.getTpsFactor())) ? 255 : 0, 0, 255);
