...
import me.bopis.king.features.modules.player.BlockTweaks;
...
public class Surround
    ...
    public void onEnable() {
        ...
        if (this.center.getValue().booleanValue() && !Bopis.moduleManager.isModuleEnabled("Freecam")) {
            if (Surround.mc.world.getBlockState(new BlockPos(Surround.mc.player.getPositionVector())).getBlock() == Blocks.WEB) {
                Bopis.positionManager.setPositionPacket(Surround.mc.player.posX, this.startPos.getY(), Surround.mc.player.posZ, true, true, true);
            } else {
                Bopis.positionManager.setPositionPacket((double) this.startPos.getX() + 0.5, this.startPos.getY(), (double) this.startPos.getZ() + 0.5, true, true, true);
            }
        }
        this.retries.clear();
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.render.getValue().booleanValue() && (this.isSafe == 0 || this.isSafe == 1)) {
            ...
            for (BlockPos pos : this.placeVectors) {
                ...
                RenderUtil.drawBoxESP(pos, this.colorSync.getValue() != false ? ClickGui.getInstance().getCurrentColor() : new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue()), this.customOutline.getValue(), new Color(this.cRed.getValue(), this.cGreen.getValue(), this.cBlue.getValue(), this.cAlpha.getValue()), this.lineWidth.getValue().floatValue(), this.outline.getValue(), this.box.getValue(), this.boxAlpha.getValue(), false);
            }
        }
    }
        ...
        for (Vec3d vec3d : vec3ds) {
            ...
            switch (BlockUtil.isPositionPlaceable(position, this.raytrace.getValue())) {
                ...
                case 1: {
                    ...
                    if (!this.extendMove.getValue().booleanValue() && Bopis.speedManager.getSpeedKpH() != 0.0 || isExtending || this.extenders >= this.extender.getValue())
                        continue block6;
    ...
    private boolean check() {
        ...
        switch (this.movementMode.getValue()) {
            ...
            case LIMIT: {
                if (!(Bopis.speedManager.getSpeedKpH() > this.speed.getValue())) break;
                return true;
            }
            case OFF: {
                if (!(Bopis.speedManager.getSpeedKpH() > this.speed.getValue())) break;
        ...
        return Bopis.moduleManager.isModuleEnabled("Freecam") || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Surround.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockObsidian.class);
    }
