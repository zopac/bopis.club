...
import me.bopis.king.features.modules.combat.Killaura;
import me.bopis.king.features.modules.player.FakePlayer;
import me.bopis.king.features.modules.player.Freecam;
...
public class EntityUtil implements Util {
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil.mc.player) || (entity instanceof EntityPlayer && Bopis.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && Bopis.friendManager.isFriend((EntityPlayer) entity)) {
                color = new Color(0.33333334f, 1.0f, 1.0f, alpha / 255.0f);
            }
            final Killaura killaura = Bopis.moduleManager.getModuleByClass(Killaura.class);
