...
public class ConfigManager implements Util {
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("bopis").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("bopis/" + name + "/"))) {
            this.config = "bopis/" + name + "/";
        } else {
            this.config = "bopis/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("bopis").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("bopis/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("bopis/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("bopis", ""));
                writer.close();
            } else {
